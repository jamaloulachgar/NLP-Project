...
import { Send, Menu, Info, Sparkles, Brain } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto bg-[#F3F6FB] pb-32">
        {!hasMessages ? (
          <EmptyState lang={lang} onQuickAction={handleQuickAction} />
        ) : (
        ...
        <div className="w-full p-4 pointer-events-auto relative">
          <form onSubmit={handleSubmit}>
            <div className={cn(
              ...
              {/* Brain icon */}
              {/* <Brain className="w-5 h-5 text-[#EC4899] shrink-0" /> */}
...
function EmptyState({ lang, onQuickAction }: { lang: Language; onQuickAction: (text: string) => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center">
      ...
      <h2 className="text-xl font-semibold mb-2">{t('noMessages', lang)}</h2>
      <p className="text-muted-foreground mb-8 max-w-md">{t('askAnything', lang)}</p>
