...
import { Conversation, Message, ChatLanguage } from '@/types/chat';
import { 
  listConversations, 
  createConversation, 
  getMessages, 
...
export function useConversations() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const response = await apiSendMessage(convId, text, chatLang);
  ...
  }, [currentConversationId, chatLang, startNewConversation]);
